...
function parseAndLoad(data) {
  ...
  while ( dataArray.length > 0 )
  {
    ...
    else if (nextToken == "sum")
    {
      var args = new Array();
      // curve and sum curve indices
      args.push( parseInt(dataArray.splice(0, 1)) );
      args.push( parseInt(dataArray.splice(0, 1)) );
    ...
    else if (nextToken == "rotate")
    {
      ...
      // curve index
      args.push( parseInt(dataArray.splice(0, 1)) );
      var axis = new THREE.Vector3( parseFloat(dataArray.splice(0, 1)),
                                    parseFloat(dataArray.splice(0, 1)),
                                    parseFloat(dataArray.splice(0, 1)) );

      var dxyz = new THREE.Vector3( parseFloat(dataArray.splice(0, 1)),
                                    parseFloat(dataArray.splice(0, 1)),
                                    parseFloat(dataArray.splice(0, 1)) );
      ...
      inFileParsed.directives[0] instanceof InputDirective)
  {
    ...
    else if (inFileParsed.directives[0].type == "sum")
    {
      var curveIndex1 = inFileParsed.directives[0].args[0];
      var curveIndex2 = inFileParsed.directives[0].args[1];
      bspline1 = new THREE.NURBSCurve( inFileParsed.curves[curveIndex1].points,
                                          inFileParsed.curves[curveIndex1].knot,
                                          inFileParsed.curves[curveIndex1].order );
      bspline2 = new THREE.NURBSCurve( inFileParsed.curves[curveIndex2].points,
                                          inFileParsed.curves[curveIndex2].knot,
                                          inFileParsed.curves[curveIndex2].order );

      // 1: sum
      currentShape = new NURBSSurfaceBuilder( bspline1 , 1, bspline2, "currentShape" );
      currentShape.addToScene( scene );
    }
    else if (inFileParsed.directives[0].type == "rotate")
    {
      // NOTE: arg[0] is curve index number
      //       arg[1] is rotation vector
      //       arg[2] is dxyx vector
      var curveIndex = inFileParsed.directives[0].args[0];
      bsplineNew = new THREE.NURBSCurve( inFileParsed.curves[curveIndex].points,
                                            inFileParsed.curves[curveIndex].knot,
                                            inFileParsed.curves[curveIndex].order );
      var rotateAxis = inFileParsed.directives[0].args[1];
      var dxyz = inFileParsed.directives[0].args[2];
      var rotateArgs = new Array();
      rotateArgs.push(rotateAxis);
      rotateArgs.push(dxyz);

      // 2: rotate
      currentShape = new NURBSSurfaceBuilder( bsplineNew, 2, rotateArgs, 
                                       "currentShape" );
      currentShape.addToScene( scene );
    }
